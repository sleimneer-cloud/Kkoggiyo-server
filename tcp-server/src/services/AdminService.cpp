...
bool AdminService::processClearBanUser(int memberId, const std::string &reason, int adminId, std::string &outMsg)
{
    MYSQL *conn = DBConnectionPool::getInstance().getConnection();
    if (!conn)
    {
    ...
    {
        DBConnectionPool::getInstance().releaseConnection(conn);
        outMsg = "차단된 유저가 아닙니다."; // 이미 정상인 유저
        return false;
    }

    // 2. 차단 상태 해제 (is_banned = 0)
    std::string qUpdate = "UPDATE member SET is_banned = 0 WHERE member_id = ?";
    std::vector<std::string> pUpdate = {std::to_string(memberId)};
    if (!DBHelper::executeUpdate(conn, qUpdate, pUpdate))
    {
        DBConnectionPool::getInstance().releaseConnection(conn);
    ...
    // 3. chat_queue 조회 (Ban 로직과 동일하게 추적용 큐 확보)
    std::string qQueue = "SELECT queue_id FROM chat_queue WHERE member_id = ? ORDER BY queue_id DESC LIMIT 1";
    std::vector<std::string> pQueue = {std::to_string(memberId)};
    std::vector<std::vector<std::string>> resQueue;
    int queueId = 0;
    ...
    // 4. admin_action 기록 (action_type을 'UNBAN_MEMBER'로 기록)
    std::string qAction = "INSERT INTO admin_action (queue_id, admin_id, action_type, target_type, target_id, memo) VALUES (?, ?, 'UNBAN_MEMBER', 'MEMBER', ?, ?)";
    std::vector<std::string> pAction = {std::to_string(queueId), std::to_string(adminId), std::to_string(memberId), reason};
    DBHelper::executeUpdate(conn, qAction, pAction);

    DBConnectionPool::getInstance().releaseConnection(conn);
    outMsg = "유저 차단 해제가 완료되었습니다.";
    return true;
}